#!/usr/bin/env python3
"""
Simple PDF Generator Script
Creates a basic PDF document with sample content
"""
...
def create_pdf():
    ...
    print(f"File size will be generated when saved.")
    return output_file

def create_minimal_pdf():
    """Create a minimal PDF for quick testing"""
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font('Arial', 'B', 16)
    pdf.cell(0, 10, 'Minimal PDF Example', ln=True, align='C')
    pdf.ln(10)
    pdf.set_font('Arial', '', 12)
    pdf.multi_cell(0, 10, 'This is a very simple PDF created with Python.')
    pdf.output('minimal.pdf')
    print("Minimal PDF created: minimal.pdf")

if __name__ == "__main__":
    print("PDF Generator Script")
    print("=" * 40)
    print("Select an option:")
    print("1. Create detailed PDF document")
    print("2. Create minimal PDF for testing")
    print("3. Exit")
    ...
    try:
        ...
        if choice == "1":
            ...
            print("Invalid choice. Creating detailed PDF by default...")
            create_pdf()
        ...
        print(f"Error: {e}")
        print("Creating minimal PDF instead...")
        create_minimal_pdf()